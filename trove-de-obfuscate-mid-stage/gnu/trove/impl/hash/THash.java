...
public abstract class THash implements Externalizable {
   ...
   public THash(int var1, float var2) {
      ...
      this.setUp(gnu.trove.impl.HashFunctions.b((float)var1 / var2));
   }
   ...
   public void ensureCapacity(int var1) {
      if (var1 > this._maxSize - this.size()) {
         this.rehash(gnu.trove.impl.PrimeFinder.nextPrime(Math.max(this.size() + 1, gnu.trove.impl.HashFunctions.b((float)(var1 + this.size()) / this._loadFactor) + 1)));
   ...
   public void compact() {
      this.rehash(gnu.trove.impl.PrimeFinder.nextPrime(Math.max(this._size + 1, gnu.trove.impl.HashFunctions.b((float)this.size() / this._loadFactor) + 1)));
      this.computeMaxSize(this.capacity());
   ...
   protected int setUp(int var1) {
      int var2 = gnu.trove.impl.PrimeFinder.nextPrime(var1);
   ...
   protected final void postInsertHook(boolean var1) {
      ...
      if (++this._size > this._maxSize || this._free == 0) {
         int var2 = this._size > this._maxSize ? gnu.trove.impl.PrimeFinder.nextPrime(this.capacity() << 1) : this.capacity();
         this.rehash(var2);
         this.computeMaxSize(this.capacity());
      }
   ...
   public void writeExternal(ObjectOutput var1) {
       try {
           ...
           Logger.getLogger(THash.class.getName()).log(Level.SEVERE, null, ex);
       }
   }
